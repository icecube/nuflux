...
class PyFluxFunction: public nuflux::FluxFunction {
  public:
    ...
    double getMinEnergy() const override{
      PYBIND11_OVERRIDE(double, FluxFunction, getMinEnergy);
    }
    virtual double getMaxEnergy() const override{
      PYBIND11_OVERRIDE(double, FluxFunction, getMaxEnergy);
      }
    virtual double readExtents(nuflux::ParticleType type) const override{
      PYBIND11_OVERRIDE(double, FluxFunction, readExtents, type);
    }
};

PYBIND11_MODULE(_nuflux, m) {
  ...
  m.def("printModels", &nuflux::printModels, "Print a list of models to stdout");
