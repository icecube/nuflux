...
#include <nuflux/IPSGLEFlux.h>

namespace nuflux{
  boost::shared_ptr<FluxFunction> IntegralPreservingFluxSubGeV::makeFlux(const std::string& fluxName){
    return(boost::dynamic_pointer_cast<FluxFunction>(boost::make_shared<IntegralPreservingFluxSubGeV>(fluxName)));
  }

  IntegralPreservingFluxSubGeV::IntegralPreservingFluxSubGeV(const std::string& fluxName):
  ...
  IntegralPreservingFluxSubGeV::~IntegralPreservingFluxSubGeV(){}
  
  void IntegralPreservingFluxSubGeV::loadTables(const std::string& fluxName, ParticleType type){
    const double enBinWidth=.05;
    {
      std::string fname = detail::getDataPath("IPSGLEFlux/"+fluxName+"3D.dat");
    ...
    {
      std::string fname = detail::getDataPath("IPSGLEFlux/"+fluxName+"2D.dat");
  ...
  double IntegralPreservingFluxSubGeV::getMinEnergy() const{
    return 1e-1;
  }

  double IntegralPreservingFluxSubGeV::getMaxEnergy() const{
    return 1e4;
  }  
  
  
  double IntegralPreservingFluxSubGeV::getFlux(ParticleType type, double energy, double cosZenith) const{
  ...
  double IntegralPreservingFluxSubGeV::getFlux(ParticleType type, double energy, double azimuth, double cosZenith) const{
  ...
  double IntegralPreservingFluxSubGeV::evaluate2D(ParticleType type, double energy, double cosZenith) const{
  ...
  double IntegralPreservingFluxSubGeV::InterpolateAzimuth(ParticleType type, double energy, double azimuth, double cosZenith) const{
  ...
  double IntegralPreservingFluxSubGeV::InterpolateCZFlux(ParticleType type, double energy, double azimuth, double cosZenith) const{
    double logEnergy=log10(energy);
    dumbHistogram hFluxVCosZenith(-1);
...
NNF_REGISTER_FLUX("IPSGhonda2014_spl_solmin",&nuflux::IntegralPreservingFluxSubGeV::makeFlux);
NNF_REGISTER_FLUX("IPSGhonda2006_sno_solmin",&nuflux::IntegralPreservingFluxSubGeV::makeFlux);
NNF_REGISTER_FLUX("IPSGhonda2014_sk_solmin", &nuflux::IntegralPreservingFluxSubGeV::makeFlux);
